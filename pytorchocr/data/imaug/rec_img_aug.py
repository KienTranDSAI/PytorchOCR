...
from .hkt_augment import HKTAugment, EvalHKTAugment,HKTAugmentDetection
...
class RecAug(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.hkt_augment = HKTAugment()

    def __call__(self, data):
        img = data["image"]
        h, w, _ = img.shape
        # HKT Augment
        # folder_path = "/raid/kientdt/shared_drive_cv/ocr/kientdt/PaddleOCR/training_logv5"
        # if not os.path.exists(folder_path):
        #     os.mkdir(folder_path)
        img, name = self.hkt_augment(img)
        # cv2.imwrite(folder_path + "/{0}_{1}.jpg".format(name, random.randint(0, 9999)), img)
        # tia
...
class DetAug(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.hkt_augment_detection = HKTAugmentDetection()
    ...
    def __call__(self, data):
        ...
        # if not os.path.exists(folder_path):
        #     os.mkdir(folder_path)
        img, name = self.hkt_augment_detection(img)
        # cv2.imwrite(folder_path + "/{0}_{1}.jpg".format(name, random.randint(0, 9999)), img)
        # tia
        # if random.random() <= self.tia_prob:
        #     if h >= 20 and w >= 20:
